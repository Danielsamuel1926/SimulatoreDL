...
from fpdf import FPDF # fpdf è l'alias per fpdf2
import base64

# ==============================
# FUNZIONE DI GENERAZIONE PDF (NUOVA SEZIONE)
# ==============================
...
def genera_pdf_simulazione(cliente, periodo, tipo_energia, offerta, df_risultati, totale_finale, fatt_attuale, diff):
    pdf = PDF()
    pdf.alias_nb_pages()
    pdf.add_page()
    ...
    pdf.cell(0, 7, f'Periodo Analizzato: {periodo}', 0, 1)
    pdf.ln(8)
    ...
    for index, row in df_risultati.iterrows():
        ...
        pdf.cell(col_widths[0], 6, row['Descrizione'], 1, 0, 'L')
...
# ==============================
# STILE GENERALE
# ... (il codice di stile e le costanti rimangono invariate) ...
# ==============================

# ... (Omesso codice di stile, header, menu e costanti per brevità) ...
# ==============================
# STILE GENERALE
# ==============================
...
# ==============================
st.markdown("---")
col1, col2, col3 = st.columns(3) # Aggiungo col3 per il pulsante download
...
        # ==================================================
        # LOGICA DI DOWNLOAD PDF (NUOVA SEZIONE)
        # ==================================================
        
        # Genera i byte del PDF chiamando la funzione
        pdf_output = genera_pdf_simulazione(
            cliente,
            f"{mese1} {f'- {mese2}' if periodo == 'Bimestrale' else ''}",
            tipo,
            offerta,
